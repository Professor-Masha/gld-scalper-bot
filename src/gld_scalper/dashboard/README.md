...
- **Memory Graph:** a native JavaFX, stable open-ring map built from bounded SQLite summaries and immutable model manifests. It connects the current decision to evidence, playbooks, candidates, champions, training runs, datasets, outcomes, LLM reviews, and safety state. Filters and inspectors are analytical only.
...
- `/api/v1/llm/status`: compact provider runtime state plus independent managed states, bounded activity, and latest structured results for a focused `llm_analysis` review and the full `llm_cycle` FinGPT pipeline. It also reports FinGPT source readiness, discovered modules, workflow stages, selected reasoning engine, and `broker_authority=false`. Provider test state is in-memory and resets to `not_tested` when the local gateway restarts; this avoids claiming stale health.
- `job_results.py`: parses the latest structured child-job result from a bounded four-MiB log tail and caches it by file timestamp and size, preventing AI status polling from rescanning an accumulated log.
...
The JavaFX AI Lab can launch hourly and daily FinGPT cycles through the same
typed, allowlisted job path used by Training. It never assembles a shell command
or imports FinGPT into the live event loop. The gateway tracks `llm_cycle` and
`llm_analysis` separately so a previous review cannot masquerade as a pipeline
result. Log parsing is bounded and cached, and status polling performs no model
