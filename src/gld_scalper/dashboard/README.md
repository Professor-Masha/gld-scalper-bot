...
- `/api/v1/llm/status`: compact provider runtime state, managed offline-review state, bounded recent activity, and the latest structured result. Provider test state is in-memory and resets to `not_tested` when the local gateway restarts; this avoids claiming stale health.
